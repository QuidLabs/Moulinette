...
interface ISTBTC {
    ...
    function unlock(address account, uint256 amount) external; // TODO unlock
}


contract IncredibleSquaringTaskManager is
    Initializable, OwnableUpgradeable,
    Pausable, BLSSignatureChecker,
    ...
    IIncredibleSquaringTaskManager
{
    ...
    function createNewTask(
        ...
        if(address(stBTC).code.length == 0) {
            stBTC = ISTBTC(); // TODO 
        }  

        if (isBurnTask) {
            stBTC.lock(msg.sender, burnAmount);
    ...
    function respondToTask(
        ...
        if(task.isBurnTask){
            stBTC.burn(task.burnFrom, taskResponse.amount);
        }
        else {
            stBTC.mint(task.mintTo,
            taskResponse.amount,
            task.btcTxHash);
        }
    }
    ...
    // TODO: should we check that quorumNumbers is same as the one recorded in the task?
    function raiseAndResolveChallenge(
        ...
        // TODO: contracts team will add this function back in the BLSSignatureChecker, which we should use to prevent potential bugs from code duplication
