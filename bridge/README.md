...
The user starts by depositing the desired amount of BTC into the specified BTC multisig address on the Bitcoin network, overseen by AVS operators. After completing the transaction, the user accesses the app to input the desired ETH Mainnet destination address, the transaction hash of the BTC deposit, and the signature of the ETH destination address, signed using their BTC private key.

To verify the BTC transaction, operators employ Bitcoin light nodes. Once they reach a consensus, the AVS service transfers the corresponding amount of stBTC ERC20 tokens to the user's account on the ETH mainnet, minus the bridging fees, which serve as compensation for the operators and restakers.

When a user wants to convert their stBTC back to native BTC, the process is similar. They specify the BTC destination address and the amount of stBTC to be bridged. The stBTC is then burned, and the operators sign the transaction to release BTC from the multisig address. Upon achieving consensus, the aggregated signatures and transaction are broadcast to the BTC network, releasing the native BTC to the user.

Users can track changes in their stBTC ERC20 balance and native BTC balance through their MetaMask wallet and the local BTC network instance used for testing purposes.

After setting up a `regtest` node Run the `bitcoinScripts/payment/sendMoney.js` (first run npm i in bitcoinScripts folder). It will fund the hardcoded multisig with 1 BTC, which will give your SPV node (after you set it up) a transaction that you can test. 
